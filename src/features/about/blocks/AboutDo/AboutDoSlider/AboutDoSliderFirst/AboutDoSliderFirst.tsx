...
import styles from "./AboutDoSliderFirst.module.scss";
...
function AboutDoSliderFirst() {
   ...
   useEffect(() => {
      ...
      const tweens = rows.map((row, index) => {
         ...
         // Если строка движется вправо, сразу ставим её в стартовое смещение.
         if (direction === 1) {
      ...
      <div className={styles.home_effective_content_item}>
         <div className={styles.home_effective_content_item_list}>
            {runningLinesData.map((line, i) => (
               ...
               >
                  <div
                     className={styles.home_effective_content_item_list_row_track}
                     ref={(el) => {
                        if (el) rowsRef.current[i] = el;
                     }}
                  >
                     {/* Дублируем элементы дважды для бесконечной ленты. */}
...
export default AboutDoSliderFirst;